// =====================================================
// Tela: Lista de Contas a Receber
// =====================================================
...
const FILTER_OPTIONS: { label: string; value: TransactionStatus | 'all' }[] = [
  ...
  { label: 'Recebidas', value: 'pago' },
  { label: 'Vencidas', value: 'vencido' },
];

export default function ReceberListScreen() {
    ...
    <SafeAreaView style={styles.container}>
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <TransactionCard
            transaction={item}
            onPress={() => router.push(`/(tabs)/receber/${item.id}`)}
            onMarkPaid={() => handleMarkReceived(item.id)}
          />
        )}
        ListEmptyComponent={
          <EmptyState
            ...
            subtitle="Toque no + para adicionar sua primeira receita"
          />
        }
      ...
      <TouchableOpacity
        ...
        activeOpacity={0.8}
        accessibilityRole="button"
        accessibilityLabel="Adicionar nova conta a receber"
      >
